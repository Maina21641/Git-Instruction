...
Для того, чтобы вернуться в master, нужно указать команду git checkout master

# Слияние веток.

Для того, чтобы слить две веткки, нужно указать команду git merge Название_Ветки_Которую_Хотим_Влить. Предварительно нужно проверять на какой ветке мы находимся. Для задания по 1-му семинару сливаем ветку Master с веткой Markdown_Instruction

# Удаление ветки.

После удачного слияния или из-за другой причины, ветку можно удалить командой git branch -d НазваниеВетки. Удаляем ветку Markdown_Instruction